...
class RetrievalTools:
    ...
    def __init__(
        ...
        *,
        ...
        # about_entity: memory -> concept. Index concept id -> [memory ids].
...
class DefaultAgenticController:
    ...
    Implements the concept -> facts -> self-assess -> fallback -> iterate loop
    without any LLM, so it runs reproducibly and key-free. A real LLM controller
    ...
    def retrieve(
        ...
        for it in range(1, max_iter + 1):
            # Widen the concept search each iteration.
            concept_hits = tools.search_concepts(question, concept_limit * it)
            for c in concept_hits:
                if c.concept_id not in used_concepts:
                    used_concepts.append(c.concept_id)
            linked = tools.facts_for_concepts([c.concept_id for c in concept_hits])
            _merge(tools.rank_facts(question, linked))

            ranked = sorted(best.values(), key=lambda h: (-h.score, h.object_id))
            confidence = _confidence(ranked)

            # Self-assess; fall back to direct fact search when unsure or thin.
            if confidence < threshold or len(ranked) < 3:
                _merge(tools.search_facts(question, max_facts))
                ranked = sorted(best.values(), key=lambda h: (-h.score, h.object_id))
                confidence = _confidence(ranked)

            if confidence >= threshold and len(ranked) >= 3:
                break

        ranked = sorted(best.values(), key=lambda h: (-h.score, h.object_id))
        fact_ids = [h.object_id for h in ranked[:max_facts]]
        return RetrievalDecision(
            fact_ids=fact_ids,
            confidence=confidence,
            iterations=it,
            reasoning=(
                f"concepts={len(used_concepts)} facts={len(fact_ids)} "
